...
export default async function CardData() {
  ...
  const { data: transaksi_masuk_terlama }: any = await supabse.from('transaksimasuk').select("*, obat(nama)").order('created_at', { ascending: false }).range(0, 0)
  const { data: transaksi_masuk_terbaru }: any = await supabse.from('transaksimasuk').select("*, obat(nama)").order('created_at', { ascending: true }).range(0, 0)

  const { data: transaksi_keluar_terlama }: any = await supabse.from('transaksikeluar').select("*, obat(nama)").order('created_at', { ascending: false }).range(0,0)
  const { data: transaksi_keluar_terbaru }: any = await supabse.from('transaksikeluar').select("*, obat(nama)").order('created_at', { ascending: true }).range(0,0)
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-lg shadow-md p-5 gap-3 min-w-[300px] flex flex-col">
        ...
        <p className="text-xl font-bold text-emerald-500 rounded w-fit p-1">{transaksi_masuk_terbaru[0]?.obat.nama} | {currency(transaksi_masuk_terbaru[0]?.nominal)}</p>
      ...
      <div className="bg-white rounded-lg shadow-md p-5 gap-3 min-w-[300px] flex flex-col">
        ...
        <p className="text-xl font-bold text-emerald-500 rounded w-fit p-1">{transaksi_masuk_terlama[0]?.obat.nama} | {currency(transaksi_masuk_terlama[0]?.nominal)}</p>
      ...
      <div className="bg-white rounded-lg shadow-md p-5 gap-3 min-w-[300px] flex flex-col">
        ...
        <p className="text-xl font-bold text-emerald-500 rounded w-fit p-1">{transaksi_keluar_terbaru[0]?.obat.nama} | {currency(transaksi_keluar_terbaru[0]?.nominal)}</p>
      ...
      <div className="bg-white rounded-lg shadow-md p-5 gap-3 min-w-[300px] flex flex-col col-start-2">
        ...
        <p className="text-xl font-bold text-emerald-500 rounded w-fit p-1">{transaksi_keluar_terlama[0]?.obat.nama} | {currency(transaksi_keluar_terlama[0]?.nominal)}</p>
